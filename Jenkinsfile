    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                kubectl set image deployment/flask-service flask-container=arvindsiva68/proj3:v${BUILD_NUMBER}
                '''
            }
        }
            ...
            steps {
                script{
                    if (env.GIT_BRANCH == "origin/main") {
                        ...
                        kubectl set image deployment/flask-service flask-container=arvindsiva68/proj3:prod-v${BUILD_NUMBER} -n prod
                        '''
                    } else if (env.GIT_BRANCH == "origin/dev") {
                        sh '''
                        kubectl apply -n dev -f ./kubernetes
                        kubectl set image deployment/flask-service flask-container=arvindsiva68/proj3:dev-v${BUILD_NUMBER} -n dev
                        '''
                    } else {
                        sh '''
                        echo "Branch not recognised"
